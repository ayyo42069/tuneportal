        ...
        <div class="container mx-auto px-4 py-8 mt-16">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                    <h1 class="text-3xl font-bold text-red-600 dark:text-red-400 mb-4 md:mb-0">Tuning Tools</h1>
                    
                    <!-- Category Filter -->
                    <div class="relative w-full md:w-64">
                        <select id="categoryFilter" 
                                class="w-full px-4 py-2 rounded-lg border border-red-300 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-transparent bg-white dark:bg-gray-700 dark:text-white appearance-none">
                        ...
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-red-600 dark:text-red-400">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="category-group" data-category="<?= htmlspecialchars($category) ?>">
                        <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200 border-b-2 border-red-200 dark:border-red-700 pb-2">
                            <?= htmlspecialchars($category) ?>
                        </h2>
                        <div class="space-y-4">
                            <?php foreach ($tools as $tool): ?>
                            <div class="bg-white dark:bg-gray-700 rounded-xl shadow-md hover:shadow-lg transition-shadow duration-200 p-6">
                                <div class="flex items-start justify-between mb-3">
                                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">
                                        <?= htmlspecialchars($tool['name']) ?>
                                    </h3>
                                    <span class="px-2 py-1 text-sm rounded-full bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200">
                                        <?= $tool['file_path'] ? strtoupper(pathinfo($tool['file_path'], PATHINFO_EXTENSION)) : 'URL' ?>
                                    </span>
                                </div>
                                <p class="text-gray-600 dark:text-gray-300 mb-4"><?= htmlspecialchars($tool['description']) ?></p>
                                <div class="mt-4">
